...
if(isset($_GET['source']) && isset($_GET['s_id']) && isset($_GET['img_id'])) {
	...
	$the_home_id = $_GET['s_id']; //id do conteúdo
	$the_home_image_id = $_GET['img_id']; //id da imagem

    $query = "SELECT A.TITLE, A.DESCR, A.POSITION, B.ID, B.IMG_PATH, B.MAINPAGE_DATA_ID FROM mainpage_data A, mainpage_images B ";
    $query .="WHERE A.ID = $the_home_id ";
    $query .="AND A.ID = B.MAINPAGE_DATA_ID ";
    $query .="AND B.ID = $the_home_image_id ";
		...
		$home_img_temp = $_FILES['home_img']['tmp_name'];
	  	...
	  	try {
	        ...
	        if ($_FILES['home_img']['error'] === UPLOAD_ERR_OK) {
				...
				if(empty($home_img)) {
			        $query = "SELECT * FROM mainpage_images WHERE ID = $the_home_image_id ";
		        ...
		        $query .="WHERE ID = '{$the_home_image_id}' ";    
    ...
    <div class="panel-body">
		<form action="" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
		        ...
		        <input type="number" class="form-control" name="home_position" value="<?php if(isset($home_position)){echo $home_position;}?>">
		    </div>
